        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <hr class="mb-3">
                    <form id="formInspeksi" name="formInspeksi" action="{{ route('form-retur-pt.store') }}" method="POST"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div id="keteranganAkurasiRasioTegangan" class="form-group mt-2"
                                            ...
                                            <textarea class="form-control" id="keterangan_akurasi_rasio_tegangan" name="keterangan_akurasi_rasio_tegangan"
                                                rows="2" maxlength="55" placeholder="Masukkan keterangan..."
                                                oninput="updateCharCount('keterangan_akurasi_rasio_tegangan', 'charCountAkurasiRasioTegangan')"></textarea>
                                            <small id="charCountAkurasiRasioTegangan" class="text-muted">55 karakter
                                                tersisa.</small>
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="row align-items-center">
                                            <div class="col-9">
                                                <select class="form-control" id="kelas_akurasi" name="kelas_akurasi">
                                                    ...
                                                    <option value="Kelas 3P">Kelas 3P</option>
                                                    <option value="Kelas 6P">Kelas 6P</option>
                                                </select>
                                            </div>
                                            <div class="col-3">
...
function updateKesimpulan() {
    ...
    // 1. Cek masa pakai > 40 tahun -> K8
    if (masaPakai > 40) {
        kesimpulan.value = "Bekas tidak layak pakai (K8)";
        catatanPerbaikan.textContent = "";
        return;
    }

    // 2. Cek poin B mandatory (1, 2, 3, 8)
    const kondisiRetakResin = retakResin.value;
    const kondisiNameplate = nameplate.value;
    const kondisiPenandaanTerminal = penandaanTerminal.value;
    const kondisiCoverSekunder = coverSekunder.value;

    // Jika ada yang tidak sesuai di poin B mandatory -> K8
    if (kondisiRetakResin === "Ada" || 
        kondisiNameplate === "Tidak ada" || 
        kondisiPenandaanTerminal === "Tidak ada" || 
        kondisiCoverSekunder === "Tidak ada") {
        kesimpulan.value = "Bekas tidak layak pakai (K8)";
        catatanPerbaikan.textContent = "";
        return;
    }

    // 3. Cek poin C jika section C ditampilkan
    let poinCSesuai = true;
    if (sectionC.style.display === "block") {
        const kondisiPrimerSekunderGround = parseFloat(primerSekunderGround.value) || 0;
        const kondisiSekunderPrimerGround = parseFloat(sekunderPrimerGround.value) || 0;
        const kesesuaianAkurasi = document.getElementById("kesesuaian_akurasi_rasio_tegangan").checked;

        // Pengecekan:
        // C1 > 20 MΩ dan C2 sesuai (checkbox checked)
        poinCSesuai = (kondisiPrimerSekunderGround > 20 && kesesuaianAkurasi);
    }
  // 4. Cek poin B non-mandatory (4, 5) - terminal terminal
  const kondisiTerminalPrimer = terminalPrimer.value;
    const kondisiTerminalSekunder = terminalSekunder.value;
    const terminalTidakSesuai = (kondisiTerminalPrimer === "Tidak ada" || kondisiTerminalSekunder === "Tidak ada");

    // 4. Cek poin B non-mandatory (6, 7) - baut terminal
    const kondisiBautPrimer = bautPrimer.value;
    const kondisiBautSekunder = bautSekunder.value;
    const bautTidakSesuai = (kondisiBautPrimer === "Tidak ada" || kondisiBautSekunder === "Tidak ada");

    // Logika kesimpulan akhir
    if (masaPakai <= 40) {
        if (poinCSesuai) {
            if (bautTidakSesuai || terminalTidakSesuai) {
                kesimpulan.value = "Masih garansi (K7)";
...
function setupEventListeners() {
    // Poin B
    [retakResin, nameplate, penandaanTerminal, coverSekunder, 
     bautPrimer, bautSekunder].forEach(el => {
        el.addEventListener("change", updateKesimpulan);
    });

    // Poin C
    [primerSekunderGround, sekunderPrimerGround, batasKesalahan, kelasAkurasi].forEach(el => {
        el.addEventListener("change", updateKesimpulan);
    });

    // Checkbox kesesuaian akurasi
    document.getElementById("kesesuaian_akurasi_rasio_tegangan").addEventListener("change", updateKesimpulan);

    // Tahun produksi dan masa pakai
    selectTahun.addEventListener("change", updateKesimpulan);
    inputMasaPakai.addEventListener("input", updateKesimpulan);
}
